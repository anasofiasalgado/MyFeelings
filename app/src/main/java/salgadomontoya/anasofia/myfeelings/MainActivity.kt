package salgadomontoya.anasofia.myfeelings
...
import salgadomontoya.anasofia.myfeelings.utilities.CustomBarDrawable
import salgadomontoya.anasofia.myfeelings.utilities.CustomCircledDrawable
import salgadomontoya.anasofia.myfeelings.utilities.Emociones
import salgadomontoya.anasofia.myfeelings.utilities.JSONFile
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(!data){
            ...
            val fondo= CustomCircledDrawable(this, emociones)
            graph.background= fondo
            graphHappy.background =CustomBarDrawable(this, Emociones("Muy feliz", 0.0F, R.color.mustard, veryHappy))
    ...
    fun iconoMayoria(){
        ...
        if(happy>veryHappy && happy>neutral && happy>sad && happy>verySad){
            icon.setImageDrawable(resources.getDrawable(R.drawable.ic_happy_24dp))
        }
        if(veryHappy>happy && veryHappy>neutral && veryHappy>sad && veryHappy>verySad){
            icon.setImageDrawable(resources.getDrawable(R.drawable.ic_veryhappy_black_24dp))
        }
        if(neutral>veryHappy && neutral>happy && neutral>sad && neutral>verySad){
            icon.setImageDrawable(resources.getDrawable(R.drawable.ic_neutral_black_24dp))
        }
        if(sad>veryHappy && sad>happy && sad>neutral && sad>verySad){
            icon.setImageDrawable(resources.getDrawable(R.drawable.ic_sad_black_24dp))
        }
        if(verySad>veryHappy && verySad>happy && verySad>neutral && verySad>sad){
            icon.setImageDrawable(resources.getDrawable(R.drawable.ic_verysad_24dp))
        }
    ...
    fun actualizarGrafica(){
        ...
        val fondo= CustomCircledDrawable(this, lista)
